...
- Hardened external data-directory permission checks, accepted read-only system
  directories, moved RPC lock files to protected tmpfs storage, bounded token
  rotation below LuCI's RPC timeout, and aligned the official feed export with
  OpenWrt package lifecycle conventions.
